...
<meta charset="UTF-8">
<title>나라 Plus</title>
<!-- webapp >css파일을 페이지에서 불러와서 쓸 수 있도록 하는 것 -->
...
$(function(){
	...
	<!-- p-tr이라고 클릭한 정보는 this에 담겨있다는 것.. -->
	$(".p-tr").click(function(){
	
	//attr()함수
	// $(...) : 선택자, selector라고 하는 객체의 속성값을 getter하거나 setter하는 함수다.
	// $(this).attr("data-my-name", "홍길동")
	let id = $(this).attr("id") <!-- 따옴표로 묶지 않는다는 것 변수와 같은 것, 현재 클릭된 항목의 id값을 찾아서 알려달라 -->	
	//id 값을 추출할 수 있으면 몇번째 tr이 클릭되었는지 알수가 있다
	...
	// attr과 거의 같은 기능을 하는데 일부 tag나 선택자로부터 값을 추출하지 못하는
	// JS 버그를 개선한 함수
	...
	/*
		...
		>> ParseInt()
	*/
	let p_isum = su * parseInt(p_iprice) // 숫자 + 문자
	let p_osum = su * parseInt(o_iprice) // 숫자 + 문자
	let p_string = "<p>" +  p_ccode + "<br>"
						 + p_cname + "<br>"
	 					 + p_iprice+ "<br>"
	 					 + p_oprice + "<br>"
	 					 + "<span class='isum'>" + "</span>" + "<br>"
	 					 + "<span class='osum'>" + "</span>" + "<br>"
	 					 + p_isum + "/<span>" + "<br>"
	 					 + p_osum + "<br>"
	...
	// 그런데 text()함수는 일반문자열로 모든것을 취급한다. 따라서 tag를 부여해도 속성이 적용되지 않는다
	...
	alert("선택한 상품은 \n"+" 상품코드" + p_ccode  + "\n"
	 ...
	 $("#btn-total").click(function(){
		...
		// forEach와 원리가 비슷한데
		 ...
		 // item을 selector로 설정하여 >> $(item)처럼 설정
		 // 그 항목에서 text를 추출하고 숫자로 변환한 후 isum변수에 누적한다.
...
<section>
	...
	<footer> CopyRight &;copy hyang2lol@naver.com</footer>
</body>
